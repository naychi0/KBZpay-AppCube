...
const handleAutoLogin = (currentAuthToken) => {
  if (window.ma && window.ma.getAuthCode) {
    ...
    window.ma.getAuthCode({
      ...
      success: async (res) => {
        try {
          ...
          // 🌟 ဤနေရာတွင် Data ငုပ်နေတာကို ရှာပြီး ဆွဲထုတ်ပါမည် 🌟
          let extractedData = null;
          if (data?.result && Array.isArray(data.result) && data.result.length > 0) {
              const firstItem = data.result[0];
              extractedData = firstItem.result?.userInfo || firstItem.result || firstItem.userInfo || firstItem;
          } else {
              extractedData = data?.userInfo || data?.result?.userInfo || data?.result || data;
          }

          if (extractedData) {
             console.log("🎯 Final Extracted Data:", extractedData); 
             
             // ဖောင်ထဲတွင် အလိုလို ပေါ်လာစေရန် မှတ်သားခြင်း
             setUserProfile({
               name: extractedData.USER_NICKNAME || extractedData.name || extractedData.usrName || "miniapp_user",
               phone: extractedData.PLAINTEXT_MOBILE_PHONE || extractedData.phone || extractedData.mobilePhone || ""
             });
          }
    ...
    setUserProfile({ name: "Testing User", phone: "0912345678" });
  }
};
